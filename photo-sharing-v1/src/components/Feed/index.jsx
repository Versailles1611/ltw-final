...
import {
  ...
  Typography, Avatar, Divider, CircularProgress, Paper
} from '@mui/material';
import fetchModel from '../../lib/fetchModelData';

// --- CẤU HÌNH ĐỊA CHỈ BACKEND (GIỐNG CÁC FILE KHÁC) ---
const API_BASE = 'https://w2dy33-8080.csb.app'; // <--- Thay đúng ID sandbox của bạn
...
function Feed() {
  ...
  return (
    <Box sx={{ maxWidth: 800, margin: '0 auto', padding: 2 }}>
      ...
      {posts.map((post) => (
        <Card key={post._id} sx={{ mb: 4, backgroundColor: 'rgba(40,40,40,0.7)', color: '#fff' }}>
          ...
          <CardHeader
            ...
            title={
              <Link to={`/users/${post.user._id}`} style={{ textDecoration: 'none', color: '#fff', fontWeight: 'bold' }}>
          ...
          <CardContent>
            ...
            {post.comments && post.comments.map((comment) => (
              <Box key={comment._id} sx={{ mb: 1, p: 1, bgcolor: 'rgba(255,255,255,0.05)', borderRadius: 1 }}>
                <Typography variant="body2" sx={{ fontWeight: 'bold' }}>
                  <Link to={`/users/${comment.user._id}`} style={{ textDecoration: 'none', color: '#90caf9' }}>
